...
public class RedisApi {
	...
	public static void createJedisPool(String address) throws Exception {
		pool = new JedisPool(config, address.split(":")[0],
				Integer.valueOf(address.split(":")[1]), 100000);

		if (pool.isClosed() || pool == null) {
			throw new Exception("链接Redis失败!");
		}
	}
	...
	/**
	 * @Description 操作list类型数据的 @param @param key @param @param strings
	 *              list.add(object) @param @return 参数 @return Long 返回类型 @throws
	 */
